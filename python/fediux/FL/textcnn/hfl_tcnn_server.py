...
class Plaintext_Server:
    ...
    def __init__(self, method, common_params, device, client_channel):
        ...
        self.model = create_model(method, self.vocab_size, embedding_dim, num_filters, dropout, self.output_dim, device)
    ...
    def model_aggregate(self, client_models):
        ...
        for layer in server_model:
            ...
            if len(clayers) == 1:
                ...
                # 确保权重也是Float类型
                weights = self.num_examples_weights.float()
                
                server_model[layer] = torch.stack(clayers,
                                                dim=len(server_model[layer].size())) \
                                    @ weights \
                                    / self.num_examples_weights_sum.float()
        return server_model
