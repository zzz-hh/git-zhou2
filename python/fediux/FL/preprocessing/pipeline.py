from fediux.FL.utils.net_work import GrpcClient, MultiGrpcClients
from fediux.FL.utils.base import BaseModel
from fediux.FL.utils.file import save_pickle_file, load_pickle_file, save_csv_file
from fediux.FL.utils.dataset import read_data
from fediux.utils.logger_util import logger
from fediux.dataset.dataset import register_dataset
from fediux.FL.preprocessing import *
        ...
        for module_name, params in module_params.items():
            ...
            if column is None:
                ...
                if role != "server":
                    ...
                    channel.send("column_cnt", column_cnt)
                else:
                    column_cnts = channel.recv_all("column_cnt")
                    if column_cnts[0] == 0:
                        column = None
            ...
            if role != "server":
                if (
                    ...
                    elif module_name == "PCATransformer":
...
def select_module(module_name, params, FL_type, role, channel):
    ...
    elif module_name == "OutlierHandler":
        module = OutlierHandler(
            strategy=params.get("strategy", "zscore"),
    ...
    elif module_name == "ValueReplacer":
        module = ValueReplacer(
            strategy=params.get("strategy", "replace_specific"),
            replace_map=params.get("replace_map", {}),
            FL_type=FL_type,
            role=role,
            channel=channel,
        )
    ...
    elif module_name == "InterpolateImputer":
        module = InterpolateImputer(
            FL_type=FL_type,
            role=role,
            channel=channel,
        )
    elif module_name == "BFImputer":
        module = BFImputer(
            FL_type=FL_type,
            role=role,
            channel=channel,
