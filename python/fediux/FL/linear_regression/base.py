...
class LinearRegression_DPSGD(LinearRegression):
    ...
    def __init__(self, x, learning_rate=0.2, alpha=0.0001, noise_multiplier=1.0,
                 l2_norm_clip=1.0, secure_mode=True, random_seed=None):
    ...
    def add_noise(self, x, n=2):
        ...
        if self.secure_mode:
            ...
            for _ in range(2 * n):
                noise += self.rng.normal(0, self.l2_norm_clip * self.noise_multiplier, x.shape)
            noise /= np.sqrt(2 * n)
        else:
            noise = self.rng.normal(0, self.l2_norm_clip * self.noise_multiplier, x.shape)
