...
class NeuralNetworkClient(BaseModel):
    ...
    def train(self):
        ...
        try:
            ...
            logger.info("test_size is not defined,using default test_size")

        train_x=x[0:int(len(x)*(1-test_size))].copy()
        train_y=y[0:int(len(y)*(1-test_size))].copy()
        test_x=x[int(len(x)*(1-test_size)):].copy()
        test_y=y[int(len(y)*(1-test_size)):].copy()
        ...
        if method == 'Plaintext':
            client = Plaintext_Client(train_x, train_y,
                                      method,
            ...
            client = DPSGD_Client(train_x, train_y,
                                  method,
                                  self.common_params['task'],
                                  device,
                                  self.common_params['optimizer'],
        ...
        if client.output_dim == 1:
            ...
            test_y = torch.tensor(test_y.reshape(-1, 1), dtype=torch.float32)
        else:
            train_y = torch.tensor(train_y)
            test_y = torch.tensor(test_y)
        train_x = torch.tensor(train_x, dtype=torch.float32)
        test_x = torch.tensor(test_x, dtype=torch.float32)
        data_tensor = data_utils.TensorDataset(train_x, train_y)
        data_tensor_test = data_utils.TensorDataset(test_x, test_y)
        ...
        train_dataloader = DataLoader(data_tensor,
                                      ...
                                      shuffle=False)
        test_dataloader = DataLoader(data_tensor_test,
                                      batch_size=batch_size,
                                      shuffle=False)
        if method == 'DPSGD':
            client.enable_DP_training(train_dataloader)
            
        if contribution_alg and client.task == 'regression':
            if method == 'DPSGD':
                # DP data loader: Poisson sampling
                client.valid(client.train_dataloader)
            else:
                client.valid(train_dataloader)
...
class Plaintext_Client:
    ...
    def set_model(self, model, DP=False):
        if DP:
            self.model._module.load_state_dict(model)
        else:
    ...
    def send_metrics(self, dataloader):
        ...
        if self.task == 'classification':
            ...
            self.server_channel.send("metrics_keys", metrics_keys)
            for metric in metrics.keys():
                self.server_channel.send(metric, metrics[metric])
...
class DPSGD_Client(Plaintext_Client):
    ...
    def __init__(self, x, y, method, task, device,
        ...
        # 固定 Opacus 内部使用的随机数生成器
        self.dp_generator = torch.Generator()
        self.dp_generator.manual_seed(42) 
    ...
    def enable_DP_training(self, train_dataloader):
        ...
        self.model, \
            ...
            noise_generator=self.dp_generator,
        )
    ...
    def train(self, dataloader):
        ...
        if self.contribution_alg:
            ...
            while status == 0:
                # 更新模型并评估
                self.set_model(model=mid_valid_data['valid_model'], DP=True)
                self.valid(dataloader)
