# -*- coding:utf-8 -*-
# @Project   :PyCharm
# @FileName  :hfl_mobilenet_client.py
# @Time      :2025/3/11 上午10:13
# @Author    :liujiachang
...
class Client(BaseModel):
    ...
    def run(self):
        ...
        logger.info(f"Running Horizontal MobileNet algorithm, role={role_name}, process={process}")
    ...
    def train(self):
        ...
        # create dataloaders
        train_dataloader = DataLoader(train_dataset, min(train_size, self.common_params['batch_size']), shuffle=True,generator=g)
        test_dataloader = DataLoader(test_dataset, min(test_size, self.common_params['batch_size']), shuffle=True,generator=g)
        if method == 'DPSGD':
    ...
    def __init__(self, dataset, num_examples, method, device,
        ...
        for module in self.model.modules():
            if isinstance(module, torch.nn.BatchNorm2d):
                module.momentum = 0.9  # 极强依赖当前 Batch
