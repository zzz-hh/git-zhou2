...
from fediux.context import reg_dataset
from fediux.utils.logger_util import logger

import grpc
from fediux.client.ph_grpc.src.fediux.protos import service_pb2
from fediux.client.ph_grpc.src.fediux.protos import service_pb2_grpc
...
def put(df_data: pd.DataFrame, dataset_key: str = None) -> DatasetRef:
    """ Put dataset using fediux client (singltone) Dataset client.
        Default is flight client.
    """
    from fediux.client import fediux_cli
    from fediux.dataset.dataset_cli import DatasetClientFactory
    # FIXME use fediux cli client
    # dataset_client = DatasetClientFactory.create("flight", "192.168.99.23:50050", "")
    dataset_client = DatasetClientFactory.create("flight", fediux_cli.node, fediux_cli.cert)
...
def get(dataset_ref: DatasetRef) -> pd.DataFrame:
    """ Get dataset using fediux client (singltone) Dataset client.
        Default is flight client.
