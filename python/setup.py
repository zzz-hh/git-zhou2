...
def solib2sitepackage(solib_path=None):
    ...
    py_so_root_path = f"{bazel_bin_path}/src/fediux/pybind_warpper"
    ...
    so_root_path = f"{bazel_bin_path}/src/fediux/task/pybind_wrapper"
    for module_name in module_list:
...
def clean_proto():
    os.chdir("../")
    python_out_dir = "python/fediux/client/ph_grpc"
    proto_dir = python_out_dir + "/src/fediux/protos/"
...
def compile_proto():
    ...
    python_out_dir = "python/fediux/client/ph_grpc"
    grpc_python_out = "python/fediux/client/ph_grpc"

    cmd_str = """{pyexe} -m grpc_tools.protoc \
                        --python_out={python_out} \
                        --grpc_python_out={grpc_python_out} -I. \
                        src/fediux/protos/common.proto  \
                        src/fediux/protos/service.proto \
                        src/fediux/protos/worker.proto \
                        src/fediux/protos/pir.proto \
                        src/fediux/protos/psi.proto""".format(python_out=python_out_dir,
...
setup(
    name='fediux',
    version=fediux.__version__,
    description=long_description,
    author=fediux.__author__,
    author_email=fediux.__contact__,
    url=fediux.__homepage__,
