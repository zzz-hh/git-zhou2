...
// using namespace oc;
using oc::SoftSpokenShOtSender;
using oc::SoftSpokenShOtReceiver;
...
std::vector<block> MPSUParty(u32 idx, u32 numParties, u32 numElements,
                             ...
                             bool fakeBase, bool fakeTriples) {
    ...
    for (u32 i = 0; i < numParties; ++i) {
        ...
        if (i < idx) {
            chl.emplace_back(coproto::asioConnect("localhost:"
                            + std::to_string(PORT + idx * 100 + i),
                            true));
        } else if (i > idx) {
            chl.emplace_back(coproto::asioConnect("localhost:"
                            + std::to_string(PORT + i * 100 + idx),
                            false));
        }
    }
    ...
    if (idx != numParties - 1) {
        ...
        for (u32 i = 0; i < numThreads; i++) {
            encThrds[i] = std::thread([&, i]() {
                ...
                const u32 end = (i == numThreads - 1) ? numElements
                                                      : start + batch_size;
                mblock tmp;
        ...
        // construct OKVS(only mqssPMT sender need to do this,
        // i.e. party{1} to party{k-1})
        Baxos baxos;
        baxos.init(numElements, binSize, w, ssp,
                   volePSI::PaxosParam::GF128, ZeroBlock);
        ...
        for (u32 i = 0; i < numParties; ++i) {
            ...
            if (!fakeTriples && i != idx) {
                triplePath = triplePath + "./triple/triple_"
                           + std::to_string(numParties) + "_"
                           + std::to_string(numElements) + "_P"
                           + std::to_string(idx + 1) + std::to_string(i + 1);
            }
            if (i < idx) {
                mqssPMTThrds[i] = std::thread([&, i, triplePath]() {
                    mqssPMTRecv(set, cipher, choice[i],
                                chl[i], numThreads, triplePath);
                });
            } else if (i > idx) {
                mqssPMTThrds[i - 1] = std::thread([&, i, triplePath]() {
                    mqssPMTSend(numElements, cipher, choice[i - 1],
                                okvs, chl[i - 1], numThreads, triplePath);
                });
            }
        }
            ...
            if (i < idx) {
                otThrds[i] = std::thread([&, i]() {
                    ...
                    if (fakeBase) {
                        ...
                        // perform the base ot, call sync_wait
                        // to block until they have completed.
                        coproto::sync_wait(
                            base.receive(baseChoice, baseMsg, prngOT, chl[i]));
                    }
                    ...
                    // record own element's share,
                    // party who enter this branch can't be party{1}
                ...
                otThrds[i - 1] = std::thread([&, i]() {
                    ...
                    if (fakeBase) {
                        ...
                        // to block until they have completed.
                        coproto::sync_wait
                            base.send(baseMsg, prngOT, chl[i - 1]));
                    }
                    receiver.setBaseOts(baseMsg);
                    // perform random OTs, the results will be written to rMsgs
                    AlignedVector<block> rMsgs(numElements);
                    coproto::sync_wait(
                        receiver.receive(choice[i - 1],
                                         rMsgs, prngOT, chl[i - 1]));
                    // set share
                    std::copy(rMsgs.begin(), rMsgs.end(),
                              share.begin() + (i - 1) * numElements);
                });
            }
        }
    ...
    if (idx != 0) {
        ...
        std::cout << "P" << idx + 1 << " communication cost = "
                  << comm / 1024 / 1024 << " MB" << std::endl;
        // close sockets
        for (u32 i = 0; i < chl.size(); ++i) {
            chl[i].close();
        }
    ...
    // party{1} receive, reconstruct and check MAC
        ...
        std::cout << "P1 communication cost = "
                  << comm / 1024 / 1024 << " MB" << std::endl;
...
u32 MPSUCAParty(u32 idx, u32 numParties, u32 numElements,
    ...
    for (u32 i = 0; i < numParties; ++i) {
        ...
        if (i < idx) {
            chl.emplace_back(
                coproto::asioConnect(
                    "localhost:"
                    + std::to_string(PORT + idx * 100 + i), true));
        } else if (i > idx) {
            chl.emplace_back(
                coproto::asioConnect(
                    "localhost:"
                    + std::to_string(PORT + i * 100 + idx), false));
        }
    }
    ...
    if (idx != numParties - 1) {
        // lowMC encryption
        // (only mqssPMT sender need to do this, i.e. party{1} to party{k-1})
        std::vector<block> ct(numElements);
        // multithread encryption: enc(0), ..., enc(numElements - 1)
        std::vector<std::thread> encThrds(numThreads);
        const int batch_size = numElements / numThreads;
        ...
        u32 offset = (idx - 1) * numElements;
        if (idx != 0) {
            std::fill(share.begin() + offset,
                      share.begin() + offset + numElements, s);
        }
        ...
        for (u32 i = 0; i < numParties; ++i) {
            if (i < idx) {
                ...
                otThrds[i - 1] = std::thread([&, i]() {
                    ...
                    coproto::sync_wait(
                        receiver.receive(
                            choice[i - 1], rMsgs, prngOT, chl[i - 1]));
                    // set share
                    std::copy(rMsgs.begin(), rMsgs.end(),
                              share.begin() + (i - 1) * numElements);
                });
    ...
    // party{k} is always mqssPMT Receiver
        ...
        for (u32 i = 0; i < numParties - 1; ++i) {
            otThrds[i] = std::thread([&, i]() {
                ...
                AlignedVector<std::array<block, 2>> sMsgs(numElements);
                coproto::sync_wait(
                    sender.send(sMsgs, prngOT, chl[i]));
                // record own element's share
                // party who enter this branch can't be party{1}
                for (size_t j = 0; j < numElements; j++){
                    share[offset + j] ^= sMsgs[j][choice[i][j]];
