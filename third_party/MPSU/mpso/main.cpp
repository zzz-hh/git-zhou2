...
void MPSU_test(u32 idx, u32 numElements, u32 numParties,
    ...
    if (idx == 0) {
        ...
        out = MPSUParty(idx, numParties, numElements,
                        set, numThreads, fakeBase, fakeTriples);
        std::cout << "union size: " << out.size() << std::endl;
    } else {
        MPSUParty(idx, numParties, numElements,
                  set, numThreads, fakeBase, fakeTriples);
    }
}

void MPSUCA_test(u32 idx, u32 numElements, u32 numParties,
                 u32 numThreads, bool fakeBase, bool fakeTriples) {
    // generate share correlation
    ShareCorrelation sc(numParties, (numParties - 1) * numElements);
    if (!sc.exist()) {
    ...
    if (idx == 0) {
        u32 out;
        out = MPSUCAParty(idx, numParties, numElements,
                          set, numThreads, fakeBase, fakeTriples);
        std::cout << "union size: " << out << std::endl;
    } else {
        MPSUCAParty(idx, numParties, numElements,
                    set, numThreads, fakeBase, fakeTriples);
    }
}

void MPSI_test(u32 idx, u32 numElements, u32 numParties,
               u32 numThreads, bool fakeBase, bool fakeTriples) {
    // generate share correlation
    ...
    if (idx == 0) {
        ...
        out = MPSIParty(idx, numParties, numElements,
                        set, numThreads, fakeBase, fakeTriples);
        std::cout << "intersection size: " << out.size() << std::endl;
    } else {
        std::cout << "jump into MPSIParty " << idx + 1 << std::endl;
        MPSIParty(idx, numParties, numElements,
                  set, numThreads, fakeBase, fakeTriples);
    }
}

void MPSICA_test(u32 idx, u32 numElements, u32 numParties,
                 u32 numThreads, bool fakeBase, bool fakeTriples) {
    // generate share correlation
    ShareCorrelation sc(numParties, numElements);
    if (!sc.exist()) {
    ...
    if (idx == 0) {
        u32 out;
        out = MPSICAParty(idx, numParties, numElements,
                          set, numThreads, fakeBase, fakeTriples);
        std::cout << "intersection size: " << out << std::endl;
    } else {
        MPSICAParty(idx, numParties, numElements,
                    set, numThreads, fakeBase, fakeTriples);
    }
}
...
int main(int agrc, char** argv) {
    ...
    if (help) {
        ...
        std::cout
        << "    -psu:         multi-party private set union"
        << std::endl;
        std::cout
        << "    -psi:         multi-party private set intersection"
        << std::endl;
        std::cout
    << "    -psuca:       multi-party private set union cardinality"
        << std::endl;
        std::cout
        << "    -psica:       multi-party private set intersection cardinality"
        << std::endl;
        std::cout << "parameters"
        << std::endl;
        std::cout
        << "    -n:           number of elements in each set, default 1024"
        << std::endl;
        std::cout
        << "    -nn:          logarithm of the number of elements, default 10"
        << std::endl;
        std::cout
        << "    -k:           number of parties, default 3" << std::endl;
        std::cout
        << "    -nt:          number of threads, default 1" << std::endl;
        std::cout
        << "    -r:           index of party" << std::endl;
        std::cout
        << "    -genSC:       generate share correlation" << std::endl;
    ...
    if ((idx >= k || idx < 0) && !scGen) {
        std::cout
        << "wrong idx of party, please use -h to print help information"
        << std::endl;
        return 0;
    }
    ...
    if (psuTest) {
        if (scGen) {
            ...
            MPSU_test(idx, n, k, nt, fakeBase, fakeTriples);
        }
    } else if (psiTest) {
        if (scGen) {
            std::cout << "generate sc begin" << std::endl;
            ...
            MPSI_test(idx, n, k, nt, fakeBase, fakeTriples);
        }
    } else if (psucaTest) {
        if (scGen) {
            std::cout << "generate sc begin" << std::endl;
            ...
            MPSUCA_test(idx, n, k, nt, fakeBase, fakeTriples);
        }
    } else if (psicaTest) {
        if (scGen) {
            std::cout << "generate sc begin" << std::endl;
            ...
            MPSICA_test(idx, n, k, nt, fakeBase, fakeTriples);
        }
    } else {
        std::cout
        << "no protocol chosen, please use -h to print help information"
        << std::endl;
    }
    return 0;
}
