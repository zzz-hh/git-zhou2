...
void lowMCDecryptGmwSend(LowMC &cipher, mMatrix<mblock> &in,
    ...
    u64 numTriples = oc::divCeil(numElements, 128) * 128
                   * cir.mNonlinearGateCount;
    ...
    if (!triplePath.empty()) {
        ...
        if (!tripleFile.is_open()) {
            std::cout << "Error opening file " << triplePath
                      << ", using fake triples" << std::endl;
        }
    }
    for (u16 r = rounds; r > 0; --r) {
        for (u32 i = 0; i < numElements; ++i) {
            ...
            in(i, 0) =  cipher.MultiplyWithGF2Matrix(
                        cipher.invLinMatrices[r-1], in(i, 0));
    ...
    gmw.init(numElements, cir, numThreads, 0, sysRandomSeed());
        gmw.setInput(0, in);
        if (tripleFile.is_open()) {
            tripleFile.read(reinterpret_cast<char*>(a.data()),
                            a.size() * sizeof(block));
            tripleFile.read(reinterpret_cast<char*>(b.data()),
                            b.size() * sizeof(block));
            tripleFile.read(reinterpret_cast<char*>(c.data()),
                            c.size() * sizeof(block));
            tripleFile.read(reinterpret_cast<char*>(d.data()),
                            d.size() * sizeof(block));
        }
