...
void twoPartyTripleGen(u32 myIdx, u32 idx, u32 numElements,
    ...
    std::string outFileName = "./triple/" + fileName + "_"
                            + std::to_string(numElements) + "_P"
                            + std::to_string(myIdx + 1)
                            + std::to_string(idx + 1);
    ...
    for (u32 r = 0; r < rounds; r++) {
        ...
        // write A, B, C, D
        outFile.write(reinterpret_cast<char*>(gmw.mA.data()),
                      gmw.mA.size() * sizeof(block));
        outFile.write(reinterpret_cast<char*>(gmw.mB.data()),
                      gmw.mB.size() * sizeof(block));
        outFile.write(reinterpret_cast<char*>(gmw.mC.data()),
                      gmw.mC.size() * sizeof(block));
        outFile.write(reinterpret_cast<char*>(gmw.mD.data()),
                      gmw.mD.size() * sizeof(block));
    }
    ...
    for (u32 i = 0; i < numParties; ++i) {
        if (i < idx) {
            tripleGenThrds[i] = std::thread([&, i]() {
                twoPartyTripleGen(idx, i, numElements,
                                  numThreads, chl[i], fileName);
            });
        } else if (i > idx) {
            tripleGenThrds[i - 1] = std::thread([&, i]() {
                twoPartyTripleGen(idx, i, numElements,
                                  numThreads, chl[i - 1], fileName);
            });
        }
    }
