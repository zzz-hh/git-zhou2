...
void Beaver_2party_test(CLP &cmd) {
    ...
    u64 numTriplesSbox = oc::divCeil(numElements * rounds, 128)
                       * 128 * sboxCircuit.mNonlinearGateCount;
    u64 numTriplesComp = oc::divCeil(numElements, 128)
                       * 128 * compCircuit.mNonlinearGateCount;
    ...
    t.init(numTriples * 2, bs, numThreads, idx ? Mode::Receiver : Mode::Sender
                                         , prng.get<block>());
    ...
    std::cout << "P" << idx + 1 << " comm cost  = "
              << chl.bytesSent() * 1.0 / 1024 / 1024 << " MB" << std::endl;
