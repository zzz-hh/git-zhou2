...
void ShareCorrelation::generate() {
    ...
    for (size_t i = 0; i < mNumParties; ++i) {
        u32 r = prng.get<u32>();
        std::shuffle(std::begin(mPi[i]), std::end(mPi[i]),
...
void ShareCorrelation::writeToFile(std::string fileName) {
    ...
    for (size_t i = 0; i < mNumParties; ++i) {
        outFile[i].write(reinterpret_cast<char*>(mPi[i].data()),
                         mPi[i].size() * sizeof(u32));
    }

    // write a for party 2 to party k
    for (size_t i = 0; i < mNumParties - 1; ++i) {
        outFile[i + 1].write(reinterpret_cast<char*>(ma[i].data()),
                             ma[i].size() * sizeof(block));
    }

    // write aprime for party 1 to party k-1
    for (size_t i = 0; i < mNumParties - 1; ++i) {
        outFile[i].write(reinterpret_cast<char*>(maprime[i].data()),
                         maprime[i].size() * sizeof(block));
    }

    // write b for party 1 to party k-1
    for (size_t i = 0; i < mNumParties - 1; ++i) {
        outFile[i].write(reinterpret_cast<char*>(mb[i].data()),
                         mb[i].size() * sizeof(block));
    }

    // write delta for party k
    outFile[mNumParties - 1].write(reinterpret_cast<char*>(mdelta.data()),
