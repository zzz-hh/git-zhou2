...
#include "src/fediux/util/network/mem_channel.h"
#include "src/fediux/common/party_config.h"
#include "src/fediux/algorithm/base.h"
#include "src/fediux/util/network/mem_channel.h"
using namespace fediux;
namespace ph_link = fediux::link;
using StorageType = fediux::network::StorageType;
...
void RunTest(rpc::Task& task_config,
  ...
  auto meta_service =
      fediux::service::MetaServiceFactory::Create(
          fediux::service::MetaServiceMode::MODE_MEMORY, node);
  auto service = std::make_shared<DatasetService>(std::move(meta_service));
  // create channel
  fediux::PartyConfig party_config("default", task_config);
  fediux::ABY3PartyConfig aby3_party_config(party_config);;
...
TEST(logistic, logistic_3pc_test) {
  ...
  auto g_storage = std::make_shared<fediux::network::StorageType>();
