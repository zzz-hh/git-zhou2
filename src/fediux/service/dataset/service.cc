 ...
 * @return std::shared_ptr<fediux::Dataset>
 */
std::shared_ptr<fediux::Dataset>
DatasetService::newDataset(std::shared_ptr<fediux::DataDriver> driver,
 ...
 */
void DatasetService::writeDataset(const std::shared_ptr<fediux::Dataset> &dataset,
...
fediux::retcode DatasetService::registerDriver(
    const std::string& dataset_id,
  ...
  return fediux::retcode::SUCCESS;
}

std::shared_ptr<fediux::DataDriver>
DatasetService::getDriver(const std::string& dataset_id, bool is_acces_info) {
  ...
  std::shared_ptr<fediux::DataDriver> driver{nullptr};
...
fediux::retcode DatasetService::unRegisterDriver(
    const std::string& dataset_id) {
    std::lock_guard<std::shared_mutex> lck(driver_mtx_);
  auto it = driver_manager_.find(dataset_id);
  if (it != driver_manager_.end()) {
