#include "src/fediux/task_engine/task_executor.h"
...
#include "src/fediux/task/semantic/factory.h"
...
retcode TaskEngine::InitCommunication() {
  using LinkFactory = fediux::network::LinkFactory;
...
retcode TaskEngine::InitDatasetSerivce() {
  ...
  using MetaServiceFactory = fediux::service::MetaServiceFactory;
...
retcode TaskEngine::UpdateStatus(rpc::TaskStatus::StatusCode code_status,
  ...
  const auto& task_config = task_request_->task();
  fediux::rpc::TaskStatus task_status;
  fediux::rpc::Empty reply;
...
retcode TaskEngine::CreateTask() {
  try {
    using TaskFactory = fediux::task::TaskFactory;
