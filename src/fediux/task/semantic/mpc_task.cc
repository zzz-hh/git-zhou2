...
#include "src/fediux/algorithm/arithmetic.h"
#include "src/fediux/algorithm/logistic.h"
#include "src/fediux/algorithm/missing_val_processing.h"
#include "src/fediux/algorithm/mpc_statistics.h"
#include "src/fediux/common/value_check_util.h"

// #if defined(__linux__) && defined(__x86_64__)
// #include "src/fediux/algorithm/cryptflow2_maxpool.h"
// #include "src/fediux/algorithm/falcon_lenet.h"
// #endif
...
MPCTask::MPCTask(const std::string &node_id, const std::string &function_name,
  ...
  if (function_name == "logistic_regression") {
    using LRExecutor = fediux::LogisticRegressionExecutor;
    ...
    using StatisticsExecutor = fediux::MPCStatisticsExecutor;
    ...
    using MissingProcess = fediux::MissingProcess;
    ...
    if (accuracy == "D32") {
      using D32Executor = fediux::ArithmeticExecutor<D32>;
      algorithm_ = std::make_shared<D32Executor>(config, dataset_service);
    } else {
      using D16Executor = fediux::ArithmeticExecutor<D16>;
...
MPCTask::MPCTask(const std::string& function_name,
  ...
  if (function_name == "mpc_statistics") {
    algorithm_ =
        std::make_shared<fediux::MPCStatisticsExecutor>(config, nullptr);
  } else if (function_name == "AbnormalProcessTask") {
    try {
      algorithm_ =
          std::make_shared<fediux::MissingProcess>(config, nullptr);
