#ifndef SRC_FEDIUX_UTIL_NETWORK_GRPC_LINK_CONTEXT_H_
#define SRC_FEDIUX_UTIL_NETWORK_GRPC_LINK_CONTEXT_H_
#include <grpc/grpc.h>
...
class GrpcChannel : public IChannel {
 public:
  GrpcChannel(const fediux::Node& node, LinkContext* link_ctx);
...
class GrpcLinkContext : public LinkContext {
 public:
  ...
  std::shared_ptr<IChannel> buildChannel(const fediux::Node& node,
                                         LinkContext* link_ctx);
  std::shared_ptr<IChannel> getChannel(const fediux::Node& node) override;
};

}  // namespace fediux::network
#endif  // SRC_FEDIUX_UTIL_NETWORK_GRPC_LINK_CONTEXT_H_
