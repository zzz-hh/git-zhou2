#ifndef SRC_FEDIUX_UTIL_NETWORK_LINK_CONTEXT_H_
#define SRC_FEDIUX_UTIL_NETWORK_LINK_CONTEXT_H_
#include <string_view>
#include <string>
...
class LinkContext {
 public:
  using StringDataQueue = fediux::ThreadSafeQueue<std::string>;
  using StringDataContainer = std::unordered_map<std::string, StringDataQueue>;
  using StatusDataQueue = fediux::ThreadSafeQueue<retcode>;
  ...
  */
  virtual std::shared_ptr<IChannel> getChannel(const fediux::Node& node) = 0;
  ...
  fediux::common::CertificateConfig& getCertificateConfig() {
    return *cert_config_;
  }
  ...
  void initCertificate(const std::string& root_ca_path,
    ...
    cert_config_ = std::make_unique<fediux::common::CertificateConfig>(
        root_ca_path, key_path, cert_path);
  }

  retcode initCertificate(const fediux::common::CertificateConfig& cert_cfg) {
    cert_config_ = std::make_unique<fediux::common::CertificateConfig>(
 ...
 protected:
  ...
  std::unique_ptr<fediux::common::CertificateConfig> cert_config_{nullptr};
...
#endif  // SRC_FEDIUX_UTIL_NETWORK_LINK_CONTEXT_H_
