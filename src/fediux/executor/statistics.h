...
public:
  ...
  virtual retcode run(std::shared_ptr<fediux::Dataset> &dataset,
  ...
  virtual retcode PlainTextDataCompute(
      std::shared_ptr<fediux::Dataset>& dataset,
      const std::vector<std::string>& columns,
      const std::map<std::string, ColumnDtype>& col_dtype,
...
public:
  ...
  retcode run(std::shared_ptr<fediux::Dataset> &dataset,
              const std::vector<std::string> &columns,
              const std::map<std::string, ColumnDtype> &col_dtype) override;
  retcode PlainTextDataCompute(std::shared_ptr<fediux::Dataset>& dataset,
      const std::vector<std::string>& columns,
      const std::map<std::string, ColumnDtype>& col_dtype,
      eMatrix<double>* result_data,
