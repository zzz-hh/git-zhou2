...
#include "src/fediux/util/network/message_interface.h"
#include "src/fediux/util/file_util.h"
#include "src/fediux/common/value_check_util.h"

using namespace rapidjson;
// using fediux::columnDtypeToString;
...
MPCStatisticsExecutor::MPCStatisticsExecutor(
  ...
  //   node_map_[party_id] =
  //       fediux::Node(node.node_id(), node.ip(), node.port(), node.use_tls());
...
int MPCStatisticsExecutor::loadParams(fediux::rpc::Task &task) {
...
#else
int MPCStatisticsExecutor::loadParams(fediux::rpc::Task &task) {return 0;}
