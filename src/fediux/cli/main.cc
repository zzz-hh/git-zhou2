...
#include "src/fediux/cli/task_config_parser.h"
#include "src/fediux/cli/dataset_config_parser.h"
...
retcode Run() {
  ...
  auto link_ctx = fediux::network::LinkFactory::createLinkContext(link_mode);
  if (use_tls) {
    auto& ca_path = cert_config_path[0];
    auto& key_path = cert_config_path[1];
    auto& cert_path = cert_config_path[2];
    fediux::common::CertificateConfig cert_cfg(ca_path, key_path, cert_path);
    link_ctx->initCertificate(cert_cfg);
  }
  // auto channel = fediux::buildChannel(peer, use_tls, cert_config);
