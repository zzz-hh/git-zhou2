...
# start fediux server
function start_fediux_server() {
  file_dir=$(dirname $(readlink -f ${file_name}))
  echo ${file_dir}
  py_fediux_path="${file_dir}/python"
  if [ ! -d "${py_fediux_path}" ]; then
    echo "${py_fediux_path} is not exists, please check ..."
    exit 1
  fi
  #check the exists of file generated by proto
  proto_file=${py_fediux_path}/fediux/client/ph_grpc/src/fediux/protos/common_pb2.py
  if [ -f "${proto_file}" ]; then
    export PYTHONPATH=${PYTHONPATH}:${py_fediux_path}
  fi
  PYTHON_BIN=python3
